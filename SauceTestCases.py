...
class TestSauceDemoAutomation_practice:
    ...
    def test_login_button(self):
        sauce_demo = SauceDemoPage(self.driver)
        WebDriverWait(self.driver, timeout=10).until(expected_conditions.element_to_be_clickable(SauceDemoPage.
                                                                                                login_button))
        sauce_demo.click_login_button()
        login_field = self.driver.find_element(*SauceDemoPage.login_button)
        assert login_field.is_displayed(), f"Error 'Click' No se identifica el boton de inicio"
